...
open class TimeEditCommands @Autowired constructor(executor: TimeExecutor,
		...
		EditableCommands<TimeEntry, TimeData, DummyTime>(executor, terminal) {

	override val id: String = "time"

	private val successMessage = "Successfully added a time entry."
	...
	fun editTime(@CliOption(key = ["", "i", "index"],
			mandatory = true,
			help = "Index of the task to edit.") index: Int,
				 @CliOption(key = ["hours", "h"],
						 help = "Duration in hours.") hours: Long?,
				 @CliOption(key = ["minutes", "m", "mins"],
						 help = "Duration in minutes.") mins: Long?,
				 @CliOption(key = ["seconds", "s", "mins"],
						 help = "Duration in seconds.") seconds: Long?,
				 @CliOption(key = ["category", "cat", "c"],
						 help = "Category of the time entry.") name: String?,
				 @CliOption(key = ["message", "msg"],
						 help = "Note for this tracking.") message: String?,
				 @CliOption(key = ["date"], specifiedDefaultValue = "", unspecifiedDefaultValue = "",
